...
const REPORT_SECTIONS = [
    ...
    { key: 'game_theory_report', title: '博弈判断报告', team: '博弈团队' },
...
/** 供工作台与 ReportViewer 共用：按章节顺序取首个有正文的 key */
...
export default function ReportViewer({ reportData, activeSection }: ReportViewerProps = {}) {
    ...
    /** 未点选智能体时仍应有正文：默认展示按顺序首个已有内容的章节（与历史版「多段报告」体验一致） */
    const effectiveSection = activeSection ?? pickFirstSectionWithContent(getSectionContent)
    const activeMeta = effectiveSection ? REPORT_SECTIONS.find(s => s.key === effectiveSection) : null
    const activeContent = effectiveSection ? getSectionContent(effectiveSection) : ''
    const { isStreaming: activeStreaming } = effectiveSection ? getSectionState(effectiveSection) : { isStreaming: false }
                        ...
                        {activeMeta ? (
                            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                                {activeSection
                                    ...
                                    : `${activeMeta.team} · 已默认展示首个有内容的章节，点击上方卡片可切换`}
                            </p>
                        ) : (
                            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                                分析输出后此处展示报告；也可点击上方智能体卡片切换章节
                            </p>
                        )}
                    </div>
                    {isAnalyzing && (
