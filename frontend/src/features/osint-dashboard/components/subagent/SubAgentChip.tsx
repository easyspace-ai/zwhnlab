...
export function SubAgentChip({
  ...
  const logLines =
    status === 'running' ? w6PreviewLines(events, 8) : w6LogLines(events, 4)
  const previewRef = useRef<HTMLDivElement>(null)
  const showLivePreview = status === 'running'
  const previewLines =
    logLines.length > 0 ? logLines : showLivePreview ? [runningPlaceholder(connection, lastLine)] : []
      ...
      >
        <div className="flex w-full flex-wrap items-center gap-x-2 gap-y-1">
          ...
          {status === 'running' && progress > 0 ? (
            <span className="text-[10px] text-blue-600 dark:text-blue-400">{progress}%</span>
          ) : null}
        ...
        {previewLines.length > 0 ? (
          ...
          <p className="w-full truncate text-[11px] text-slate-500">{lastLine}</p>
        ) : null}
      </button>
    </div>
  )
