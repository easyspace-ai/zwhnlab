...
const syncGeneratedPagesToSessionPages = async (
  ...
  const generatedSlugs = new Set(
    args.generatedPages.map((page) => page.pageId).filter((pageId): pageId is string => Boolean(pageId))
  )
  const stalePageIds = existingPages
    .filter((page) => !generatedSlugs.has(page.file_slug))
    .map((page) => page.id)
  if (stalePageIds.length > 0) {
    await ctx.db.softDeleteSessionPages(args.sessionId, stalePageIds)
  }
}
