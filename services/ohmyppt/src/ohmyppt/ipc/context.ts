...
import { publishGenerationChunk } from '../../adapters/generation-chunk-bus.js'
...
import { filterExportableSessionPages } from '../tools/session-page-filter.js'
...
export function createIpcContext(
  ...
  const buildSessionGenerationSnapshot = async (
    ...
    for (const page of sessionPages) {
      ...
      if (fs.existsSync(htmlPath)) {
        htmlByPageId.set(pageId, await fs.promises.readFile(htmlPath, 'utf-8'))
      }
    }
    const visibleSessionPages = filterExportableSessionPages(
    ...
    for (const page of visibleSessionPages) {
      const pageId = page.file_slug
      const title = page.title || `第 ${page.page_number} 页`
      const htmlPath = resolveProjectPageHtmlPath(projectDir, pageId, page.html_path)
      const html =
        options?.includeHtml && htmlByPageId.has(pageId)
          ? htmlByPageId.get(pageId) || ''
          : ''
      pages.push({
        pageNumber: page.page_number,
        title,
  ...
  const emitGenerateChunk = (sessionId: string, chunk: GenerateChunkEvent): void => {
    ...
    publishGenerationChunk(sessionId, enrichedChunk)
  ...
  const EXPORT_PAGE_READY_TIMEOUT_MS = 15000
  const EXPORT_CAPTURE_SETTLE_MS = 250
  ...
  const resolveSessionPageFiles = async (
    ...
    for (const page of dedupedPages) {
      if (fs.existsSync(page.htmlPath)) {
        htmlByPageId.set(page.pageId, await fs.promises.readFile(page.htmlPath, 'utf-8'))
      }
    }
    const statusByPageId = new Map(sessionPages.map((sp) => [sp.file_slug, sp.status]))
    const exportablePages = filterExportableSessionPages(
      dedupedPages.map((page) => ({
        pageNumber: page.pageNumber,
        pageId: page.pageId,
        status: statusByPageId.get(page.pageId)
      })),
      htmlByPageId
    )
    const exportableById = new Set(exportablePages.map((page) => page.pageId))
    const candidatePages = dedupedPages.filter((page) => exportableById.has(page.pageId))
