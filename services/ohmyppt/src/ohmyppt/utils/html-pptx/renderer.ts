...
import { resetSharedPlaywrightBrowser } from '../../../adapters/playwright-browser-window.js'
...
export interface ExtractSlideFromBrowserWindowOptions {
  pageId: string
  title?: string
  printReadyTimedOut?: boolean
}
...
const isPlaywrightClosedError = (error: unknown): boolean => {
  const message = error instanceof Error ? error.message : String(error)
  return /Target page, context or browser has been closed|Playwright page is not ready|page crashed/i.test(
    message
  )
}
...
const runWithPptxBrowserWindow = async <T>(
  page: HtmlPageForPptx,
  timeoutMs: number,
  settleMs: number,
  ...
  run: (
    win: BrowserWindow,
    readyResult: { timedOut: boolean }
  ) => Promise<HtmlPageToPptxSlideResult>
): Promise<HtmlPageToPptxSlideResult> => {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      return await run(win, readyResult)
    } catch (error) {
      lastError = error
      if (attempt < 2 && isPlaywrightClosedError(error)) {
        log.warn('[export:pptx] browser closed during page export, retrying', {
          pageId: page.pageId,
          attempt,
...
export const captureSlideFromBrowserWindow = async (
  win: BrowserWindow,
  { pageId, title, printReadyTimedOut = false }: ExtractSlideFromBrowserWindowOptions
): Promise<HtmlPageToPptxSlideResult> => {
  ...
  return {
    slide,
    warning: printReadyTimedOut
      ? `页面 ${pageId} 未收到打印就绪信号，已按当前状态导出`
      : undefined
  }
}

export const extractSlideFromBrowserWindow = async (
  win: BrowserWindow,
  { pageId, title, printReadyTimedOut = false }: ExtractSlideFromBrowserWindowOptions
): Promise<HtmlPageToPptxSlideResult> => {
  ...
  const slide = normalizeExtractedHtmlToPptxSlide(extracted, title)
  ...
  const blockRects = (await win.webContents.executeJavaScript(
    COLLECT_KATEX_BLOCK_RECTS_SCRIPT,
    true
  ...
  return {
    slide,
    warning: [
      printReadyTimedOut ? `页面 ${pageId} 未收到打印就绪信号，已按当前状态导出` : '',
...
export const captureHtmlPageToPptxImageSlide = async (
  options: HtmlPageToPptxSlideOptions
): Promise<HtmlPageToPptxSlideResult> =>
  runWithPptxBrowserWindow(
    options.page,
    options.timeoutMs,
    options.settleMs,
    options.waitForPrintReadySignal,
    (win, readyResult) =>
      captureSlideFromBrowserWindow(win, {
        pageId: options.page.pageId,
        title: options.page.title,
        printReadyTimedOut: readyResult.timedOut
      })
  )
...
export const extractHtmlPageToPptxSlide = async (
  ...
  runWithPptxBrowserWindow(
    ...
    options.waitForPrintReadySignal,
    (win, readyResult) =>
      extractSlideFromBrowserWindow(win, {
        pageId: options.page.pageId,
        title: options.page.title,
        printReadyTimedOut: readyResult.timedOut
      })
