 ...
 */
function numericWidth(w, fallback, tableWidth) {
    if (typeof w === "number")
        return w;
    if (typeof w === "string") {
 ...
 */
function isSpecifiedColumnWidth(w) {
    if (typeof w === "number")
        return true;
...
export function calcTableIntrinsicSize(node) {
    ...
    const width = node.columns.reduce((sum, column) => sum + numericWidth(column.w ?? column.width, DEFAULT_TABLE_COLUMN_WIDTH), 0);
...
export function resolveColumnWidths(node, tableWidth) {
    const specifiedTotal = node.columns.reduce((sum, col) => sum + numericWidth(col.w ?? col.width, 0, tableWidth), 0);
    const unspecifiedCount = node.columns.filter((col) => !isSpecifiedColumnWidth(col.w ?? col.width)).length;
    ...
    return node.columns.map((col) => numericWidth(col.w ?? col.width, widthPerUnspecified, tableWidth));
}
