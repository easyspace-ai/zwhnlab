 ...
 */
import { allocateNextPomId, getCurrentSourceMap } from "./parseContext.js";
/**
...
export function getAttributes(node) {
    ...
    if (rawAttrs) {
        for (const [key, value] of Object.entries(rawAttrs)) {
            ...
            if (SOURCE_POS_ATTR_KEYS.has(attrName))
                continue;
 ...
 */
function readRawSourcePos(node) {
    const rawAttrs = node[":@"];
    if (!rawAttrs)
        return { line: undefined, file: undefined };
    return {
 ...
 */
export function formatErrorAt(node, message) {
 ...
 */
export function registerSourcePosForElement(node) {
...
export function getRawChildren(node) {
    const tagName = getTagName(node);
    return node[tagName] ?? [];
}
export function parseClassNames(attrs) {
