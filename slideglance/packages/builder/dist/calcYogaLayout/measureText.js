import { measureTextWidth as measureTextWidthOpentype, pickBundledFontForText, } from "./fontLoader.js";
/**
 ...
 */
const LATIN_CHAR_EM_WIDTH = (() => {
    const map = {};
    for (const c of "ijlI|!.,:;'`")
        map[c] = 0.28; // very narrow
    for (const c of "rtfJ-—–()[]{}")
        map[c] = 0.32; // narrow
    for (const c of "abcdeghknopqsuvxyz")
        map[c] = 0.48; // default lowercase
    for (const c of "mw")
        map[c] = 0.78; // wide lowercase
    for (const c of "ABCDEFGHKLNOPQRSTUVXYZ")
        map[c] = 0.62; // default uppercase
    for (const c of "MW")
        map[c] = 0.85; // wide uppercase
    for (const c of "0123456789")
        map[c] = 0.55; // digits (often tabular)
    map[" "] = 0.28;
    map["?"] = 0.5;
    map["%"] = 0.78;
    map["@"] = 0.85;
...
function measureTextWithOpentype(text, maxWidthPx, opts) {
    ...
    const lines = wrapText(text, maxWidthPx, (t) => measureTextWidthOpentype(t, lookupFamily, opts.fontSizePx, fontWeight, opts.measurer));
...
function splitForWrap(text) {
    ...
    // valid wrap).
    const hasCJK = /[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}\p{Script=Hangul}]/u.test(text);
    if (hasCJK) {
