...
import { DEFAULT_FONT_FAMILY } from "../defaultTextStyle.js";
...
import { PRETENDARD_REGULAR_BASE64 } from "./fonts/pretendardRegular.js";
import { PRETENDARD_BOLD_BASE64 } from "./fonts/pretendardBold.js";
const BUNDLED_FONTS = {
    "Noto Sans JP": {
 ...
 */
function getBundledFontBytes() {
    if (cachedBundledFontBytes)
        return cachedBundledFontBytes;
    const fonts = [];
 ...
 */
export function createMeasurer(extraFonts) {
    if (!extraFonts || extraFonts.length === 0)
        return getMeasurer();
    const fonts = [...getBundledFontBytes(), ...extraFonts];
 ...
 */
export function pickBundledFontForText(fontFamily, text) {
 ...
 */
export function measureTextWidth(text, fontFamily, fontSizePx, weight, measurer) {
    const m = measurer ?? getMeasurer();
    return m.measureWidth(text, fontSizePx * PT_PER_PX, weight === "bold", false, fontFamily, null);
}
/**
 ...
 */
export function measureFontLineHeightRatio(fontFamily, weight, measurer) {
    const m = measurer ?? getMeasurer();
