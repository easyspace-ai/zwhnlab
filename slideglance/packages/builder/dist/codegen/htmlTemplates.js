...
export function highlightXml(src) {
    ...
    let out = safe.replace(/&lt;(\/?)([A-Za-z][\w]*)((?:\s+.+?)?)(\s*\/?)&gt;/g, (_m, slash, tag, attrs, tail) => {
        const inside = attrs ? highlightAttrs(attrs) : "";
        return (`<span class="tk-punct">&lt;${slash}</span>` +
...
function highlightAttrs(s) {
    ...
    return s.replace(/\s+([\w.-]+)=(&quot;.*?&quot;)/g, (_m, name, value) => ` <span class="tk-attr">${name}</span>=` +
 ...
 * <code> without a link.
 */
export function childrenTable(children, pageTags) {
    ...
    const rows = entries
        ...
        const doc = escapeHtml(spec.doc ?? "");
        const tagCell = pageTags && !pageTags.has(tag)
            ? `<code>&lt;${tag}&gt;</code>`
...
export function usedByList(entries) {
    if (entries.length === 0) {
