...
function applyListYogaStyle(node, yn, yoga, ctx) {
    ...
    const fontWeight = resolveTextStyleValue(n.bold, ctx.defaultTextStyle.bold, false)
        ? "bold"
        : "normal";
    const spacingMultiple = resolveTextStyleValue(n.lineHeight, ctx.defaultTextStyle.lineHeight, 1.0);
    const fontMetricsRatio = measureFontLineHeightRatio(fontFamily, fontWeight, ctx.measurer);
