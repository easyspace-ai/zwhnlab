 ...
 */
import { SG_CXN_PREFIX, SG_ID_PREFIX, parseCxnSigil, parseIdSigil, stripSigilsByPrefix, } from "./sigils.js";
import { lookupSideIdx, isKnownPrstForCxn } from "./idxTable.js";
import { rewriteSlideGroups } from "./groupSp.js";
 ...
 * nothing changed).
 */
function rewriteSlide(parsed, diagnostics) {
 ...
 */
function rewriteSpToCxnSp(element, sig, fromMeta, toMeta) {
    ...
    for (const child of spChildren) {
        ...
        if (tag === "p:nvSpPr") {
            ...
            for (let i = 0; i < inner.length; i++) {
                ...
                const newInner = [stCxn, endCxn, ...cSpChildren];
                const replacement = { "p:cNvCxnSpPr": newInner };
 ...
 */
export async function postProcessConnectors(bytes) {
