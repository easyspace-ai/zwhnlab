 ...
 */
import { SG_GRP_PREFIX, SG_ID_PREFIX, parseGrpSigils, stripSigils, } from "./sigils.js";
...
function removeAttr(node, key) {
    const attrs = node[":@"];
    if (attrs)
        delete attrs[`@_${key}`];
}
function findDescendant(nodes, tag) {
...
function findSpTreeChildren(root) {
    for (const n of root) {
        if (!isElementNode(n))
            continue;
        if (getTag(n) !== "p:sld")
        ...
        for (const c of getChildren(n)) {
            if (!isElementNode(c) || getTag(c) !== "p:cSld")
                continue;
            for (const g of getChildren(c)) {
                if (!isElementNode(g) || getTag(g) !== "p:spTree")
                    continue;
 ...
 */
function readCNvPrName(element) {
    const cNvPr = findDescendant([element], "p:cNvPr");
    if (!cNvPr)
        return { cNvPr: undefined, name: undefined };
    return { cNvPr, name: getAttr(cNvPr, "name") };
}
function readBBox(element) {
    const spPr = findDescendant([element], "p:spPr") ??
 ...
 */
function groupAtDepth(element, depth) {
 ...
 */
function buildGrpSpElement(groupId, members, spIdCounter) {
    const elementMembers = members.filter(isElementNode);
    const bbox = unionBBox(elementMembers
        .map((m) => readBBox(m))
        .filter((b) => b !== null));
 ...
 */
function buildGroupedChildren(siblings, depth, spIdCounter, flag) {
    const result = [];
    let i = 0;
    while (i < siblings.length) {
        ...
        while (j < siblings.length) {
            ...
            if (!next || !isElementNode(next))
                break;
            if (groupAtDepth(next, depth) !== groupId)
                break;
            j += 1;
        }
 ...
 */
function nextSpIdFromSlide(parsed) {
    let max = 0;
    const walk = (nodes) => {
        for (const n of nodes) {
            if (!isElementNode(n))
 ...
 */
function stripAllRemainingSigils(parent) {
    const walk = (nodes) => {
        for (const n of nodes) {
            if (!isElementNode(n))
                continue;
 ...
 */
export function rewriteSlideGroups(parsed, _diagnostics) {
