...
function findToken(tokens, prefix) {
    return tokens.find((t) => t.startsWith(prefix));
}
function findAllTokens(tokens, prefix) {
    return tokens.filter((t) => t.startsWith(prefix));
}
function isSgToken(token) {
 ...
 */
export function buildObjectName(tokens) {
    return tokens
 ...
 */
export function parseIdSigil(name) {
 ...
 * place.
 */
export function parseCxnSigil(name) {
    const tokens = splitTokens(name);
 ...
 */
export function parseGrpSigils(name) {
    const tokens = splitTokens(name);
    return findAllTokens(tokens, SG_GRP_PREFIX).map((t) => t.slice(SG_GRP_PREFIX.length));
}
/**
 ...
 */
export function stripSigils(name) {
    const tokens = splitTokens(name);
 ...
 */
export function stripSigilsByPrefix(name, prefixes) {
    const tokens = splitTokens(name);
