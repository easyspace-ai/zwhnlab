...
import { renderBackgroundAndBorder, shouldEmbedBackgroundInText, } from "./utils/backgroundBorder.js";
import { getNodeDef } from "../registry/index.js";
import { builderObjectName } from "./utils/objectName.js";
/**
 ...
 */
function wrapCtxWithObjectName(ctx, node) {
 ...
 */
function convertMasterObject(obj, buildContext) {
    ...
    switch (obj.type) {
                ...
                text: {
                    ...
                    options: {
                        ...
                        fontSize: obj.fontSize !== undefined ||
                            ...
                            ? pxToPt(resolveTextStyleValue(obj.fontSize, defaultTextStyle.fontSize, 24))
                            : undefined,
                        ...
                        charSpacing: obj.letterSpacing !== undefined
        ...
        case "rect": {
            const opacityTransparency = obj.opacity !== undefined ? (1 - obj.opacity) * 100 : undefined;
            const explicitTransparency = obj.fill?.transparency !== undefined
...
function getRootBackground(node, buildContext) {
    const rootBackgroundColor = node.type !== "line" ? node.backgroundColor : undefined;
    const rootHasOpacity = node.type !== "line" && "opacity" in node && node.opacity !== undefined;
    const rootBackgroundImage = node.type !== "line" ? node.backgroundImage : undefined;
    let background;
...
function renderPositionedTree(data, ctx, applyRootBackground = true) {
    const { rootBackgroundColor, rootBackgroundImage, rootHasOpacity } = getRootBackground(data, ctx.buildContext);
    ...
    const resolveGroupId = (raw) => raw === "true" ? `auto-grp-${nextAutoGroupId++}` : raw;
    function renderNode(node, groupStack, isRoot = false) {
        ...
        const nextStack = typeof node.group === "string" && node.group.length > 0
        ...
        // inherits it.
        const groupedCtx = nextStack.length === (ctx.groupStack?.length ?? 0) &&
 ...
 */
function shouldEmitContainerSurface(node) {
    if (node.type === "line")
        return false;
...
function collectMasterObjects(nodes, pptx, buildContext, explicitBackground) {
    const objects = [];
    let derivedBackground = explicitBackground;
    const collectorSlide = {
        ...
        addShape(shapeName, options = {}) {
            ...
            objects.push({
                text: {
                    text: "",
                    options: { ...options, shape: normalizedShapeName },
                },
            });
            return this;
        },
...
function toMasterBackgroundProps(background) {
    if (!background)
        return undefined;
    if ("color" in background)
        return { color: background.color };
    if ("path" in background)
        return { path: background.path };
    if ("data" in background)
        return { data: background.data };
    return background;
}
/**
 * Calls pptxgenjs defineSlideMaster from a SlideMasterOptions object.
 */
function defineSlideMasterFromOptions(pptx, master, buildContext, renderedObjects, backgroundOverride) {
    const masterName = master.title || DEFAULT_MASTER_NAME;
    const masterProps = {
        title: masterName,
    };
    // Convert background
    masterProps.background = toMasterBackgroundProps(master.background ?? backgroundOverride);
    ...
    // Convert objects
    const masterObjects = master.objects?.map((obj) => convertMasterObject(obj, buildContext));
        ...
        masterProps.slideNumber = {
            ...
            fontSize: master.slideNumber.fontSize
                ...
                : buildContext.defaultTextStyle.fontSize !== undefined
                    ...
                    : undefined,
            fontFace: resolveFontFamily(master.slideNumber.fontFamily, buildContext.defaultTextStyle),
 ...
 */
export async function renderPptx(pages, slidePx, buildContext, masters, defaultMasterName, masterContents, docProps) {
    ...
    pptx.layout = "custom";
    if (docProps) {
        if (docProps.title !== undefined)
            pptx.title = docProps.title;
        if (docProps.author !== undefined)
            pptx.author = docProps.author;
        if (docProps.company !== undefined)
            pptx.company = docProps.company;
        if (docProps.subject !== undefined)
            pptx.subject = docProps.subject;
    }
    if (masters) {
        for (const master of masters) {
            const contentNodes = master.title && masterContents
                ? masterContents[master.title]
                : undefined;
            const rendered = contentNodes
                ? collectMasterObjects(contentNodes, pptx, buildContext, toMasterBackgroundProps(master.background))
                : undefined;
            defineSlideMasterFromOptions(pptx, master, buildContext, rendered?.objects, rendered?.background);
        }
    }
    for (const data of pages) {
        ...
        const ctx = { slide, pptx, buildContext, idIndex };
 ...
 * case).
 */
function buildSlideIdIndex(root) {
    const map = new Map();
    const walk = (node) => {
        if (node.id !== undefined &&
            node.type !== "connector" &&
            !map.has(node.id)) {
            const prst = node.type === "shape" ? node.shapeType : pseudoPrstFor(node.type);
            map.set(node.id, {
                x: node.x,
                y: node.y,
                w: node.w,
