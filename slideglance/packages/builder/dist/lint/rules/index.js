...
import { measureTextWidth as measureTextWidthRaw, pickBundledFontForText, } from "../../calcYogaLayout/fontLoader.js";
import { getChildren, isTextBearing, nodeIdOf, nodeSourcePos, walk, } from "../walker.js";
import { scanRawXmlForBareLtGt } from "./rawXml.js";
...
function diag(code, severity, ctx, node, path, message, extra = {}) {
...
const rawLtGtInAttr = {
    ...
    phase: "parse",
    check(ctx) {
        if (!ctx.rawXmlSources || ctx.rawXmlSources.length === 0)
            return [];
        const out = [];
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (max > OVERFLOW_TOLERANCE_PX) {
                out.push(diag("OUT_OF_PAGE", "error", ctx, node, path, `<${node.type}> spills past the slide canvas by ${Math.round(max)}px ` +
...
const outOfParent = {
    ...
    check(ctx) {
        ...
        for (const { node, parent, path } of walk(ctx.tree)) {
            ...
            if (max > OVERFLOW_TOLERANCE_PX) {
                out.push(diag("OUT_OF_PARENT", "error", ctx, node, path, `<${node.type}> overflows its ${parent.type} parent by ${Math.round(max)}px. ` +
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (node.w === 0 || node.h === 0) {
                out.push(diag("ZERO_DIM", "warn", ctx, node, path, `<${node.type}> resolved to ${node.w}x${node.h} — it renders but is invisible. ` +
...
const negativeDim = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            if (node.w < 0 || node.h < 0) {
                out.push(diag("NEGATIVE_DIM", "error", ctx, node, path, `<${node.type}> resolved to negative dimensions ${node.w}x${node.h}. ` +
                    `Usually padding > parent size. Reduce padding or widen the parent.`, { context: { w: node.w, h: node.h } }));
            }
        }
        return out;
    },
 ...
 */
function reMeasure(ctx, node, maxWidthPx) {
    if (node.type !== "text")
        return null;
    ...
    const lineHeight = node.lineHeight ?? 1.0;
    const fontFamily = node.fontFamily ?? "Inter";
    const nodeBold = !!node.bold;
    const hasBoldRun = node.runs?.some((r) => r.bold) ??
        false;
    const fontWeight = nodeBold || hasBoldRun ? "bold" : "normal";
    try {
        const r = measureText(text, maxWidthPx, {
            fontFamily,
            fontSizePx: fontSize,
            lineHeight,
            fontWeight,
...
const textOverflowV = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            const estLines = Math.max(1, Math.round(r.heightPx / (r.fontSize * r.lineHeight)));
            if (r.heightPx > node.h * TEXT_OVERFLOW_FACTOR) {
                out.push(diag("TEXT_OVERFLOW_V", "warn", ctx, node, path, `<Text> opentype re-measure ${estLines} lines need ~${Math.round(r.heightPx)}px but ` +
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (overshootPx > OVERFLOW_TOLERANCE_PX) {
                out.push(diag("TEXT_OVERFLOW_H", "warn", ctx, node, path, `<Text> natural single-line width ~${Math.round(natural.widthPx)}px exceeds box width ${Math.round(node.w)}px by ${Math.round(overshootPx)}px. ` +
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            try {
                ...
                const raw = measureTextWidthRaw(trimmed, lookup, fontSize, nodeBold ? "bold" : "normal", ctx.measurer);
            ...
            if (charsPerLine < 2) {
                out.push(diag("TEXT_WRAP_TO_1CH", "error", ctx, node, path, `<Text> is wrapping to ${charsPerLine} character per line ` +
                    ...
                    suggestedFix: {
                        ...
                        notes: "Pin the squeezed column to an intrinsic-friendly width so the w='max' sibling absorbs slack instead.",
                    },
                }));
            }
        }
        ...
        for (const { node, parent, path } of walk(ctx.tree)) {
            ...
            for (const e of ends) {
                if (e.x < parent.x - 1 ||
                    ...
                    out.push(diag("LINE_OVER_PARENT", "warn", ctx, node, path, `<Line> endpoint (${e.x.toFixed(0)},${e.y.toFixed(0)}) sits outside its ${parent.type} parent. ` +
...
const imageMissing = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (src && !data) {
                out.push(diag("IMAGE_MISSING", "error", ctx, node, path, `<Image src="${src}"> failed to resolve — no bytes were attached for rendering.`, { context: { src } }));
            }
        }
        return out;
    },
...
// ============================================================================
function rowChildren(node) {
    if (node.type !== "hstack")
        return [];
    const kids = getChildren(node);
    return kids ? kids.filter(isTextBearing) : [];
}
const inflatedLineHeight = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            const lhs = texts.map((t) => t.lineHeight ?? 1.0);
            const minLh = Math.min(...lhs);
            const maxLh = Math.max(...lhs);
            if (maxLh >= 1.3 && maxLh - minLh > 0.1) {
                out.push(diag("INFLATED_LINE_HEIGHT_IN_ROW", "warn", ctx, node, path, `Row mixes lineHeights ${lhs.map((n) => n.toFixed(2)).join(" / ")}. ` +
...
const anchorInconsistent = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (texts.length < 2)
                continue;
            const anchors = texts.map((t) => t.textVAlign ??
                "top");
            const uniq = new Set(anchors);
            if (uniq.size > 1) {
                out.push(diag("ANCHOR_INCONSISTENT", "warn", ctx, node, path, `Row siblings have mixed textVAlign anchors ${[...uniq].join(" / ")}. ` +
...
const baselineMixInRow = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (texts.length < 2)
                continue;
            const fontSizes = texts.map((t) => t.fontSize ?? 13);
            const minFs = Math.min(...fontSizes);
            const maxFs = Math.max(...fontSizes);
            if (maxFs - minFs < 2)
                continue; // sizes similar enough, no concern
            const anchors = texts.map((t) => t.textVAlign ??
                "top");
            ...
            if (!allMiddle || !lhTight) {
                out.push(diag("BASELINE_MIX_IN_ROW", "warn", ctx, node, path, `Row mixes fontSize ${fontSizes.join(" / ")} but does not apply the "mixed-size text rows" idiom ` +
...
const overlapLayer = {
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            const candidates = kids.filter((k) => (k.type !== "shape" && k.type !== "line") || (k.w > 6 && k.h > 6));
            for (let i = 0; i < candidates.length; i++) {
                for (let j = i + 1; j < candidates.length; j++) {
                    ...
                    if (smaller > 0 && inter / smaller > 0.5) {
                        out.push(diag("OVERLAP_LAYER", "info", ctx, node, path, `<Layer> children <${a.type}> and <${b.type}> overlap by >50% of the smaller bbox. ` +
...
const lowContrast = {
    ...
    check(ctx) {
        ...
        function visit(node, bg, path) {
            const ownBg = node.backgroundColor;
            const effBg = ownBg ?? bg;
            if ((node.type === "text" || node.type === "ul" || node.type === "ol") &&
                effBg) {
                ...
                if (color &&
                    ...
                    if (ratio < 4.5) {
                        out.push(diag("LOW_CONTRAST", "info", ctx, node, path, `<${node.type}> text color #${color} against background #${effBg} has contrast ratio ${ratio.toFixed(2)} — below WCAG AA (4.5).`, {
                            context: {
                                color,
                                bg: effBg,
...
const unusedStyle = {
    ...
    phase: "post-layout",
    check(ctx) {
        if (!ctx.declaredStyles || !ctx.referencedStyles)
            return [];
        if (ctx.slideIndex !== 1)
            return []; // emit only once per deck
        const out = [];
...
const unusedTemplate = {
    ...
    phase: "post-layout",
    check(ctx) {
        if (!ctx.declaredTemplates || !ctx.referencedTemplates)
            return [];
        if (ctx.slideIndex !== 1)
            return [];
        const out = [];
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (!alt || alt.trim().length === 0) {
                out.push(diag("IMG_NO_ALT", "warn", ctx, node, path, `<Image> without altText. Add altText="…" for screen readers, or decorative="true" if purely visual.`, {
                    suggestedFix: {
                        kind: "attribute-set",
                        target: "self",
                        set: { decorative: true },
                        notes: 'If the image is informative, replace this with `altText="<description>"` instead of decorative.',
                    },
                }));
            }
        }
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            for (let i = 0; i < kids.length; i++) {
                if (sourceOrder[i] !== expectedOrder[i])
                    mismatches++;
            }
            if (mismatches >= 2) {
                out.push(diag("READING_ORDER_AMBIGUOUS", "info", ctx, node, path, `<Layer> source order differs from top-to-bottom visual order at ${mismatches} positions. ` +
                    `Screen readers follow source order — reorder children if the visual flow should be read literally.`, { context: { mismatches } }));
            }
        }
        return out;
    },
...
const iconNoLabel = {
    ...
    check(ctx) {
        ...
        for (const { node, parent, path } of walk(ctx.tree)) {
            ...
            const hasNeighborText = siblings.some((s) => s !== node && isTextBearing(s));
            if (!hasNeighborText) {
                out.push(diag("ICON_NO_LABEL", "info", ctx, node, path, `<Icon> appears without an adjacent text label and is not decorative. ` +
    ...
    check(ctx) {
        ...
        for (const { node, path } of walk(ctx.tree)) {
            if (!isTextBearing(node))
                continue;
            const fsPx = node.fontSize ?? 13;
            // px to pt: pt = px * 3/4
            const fsPt = fsPx * 0.75;
            if (fsPt < TINY_FONT_PT_THRESHOLD) {
                out.push(diag("TINY_FONT", "info", ctx, node, path, `<${node.type}> fontSize=${fsPx}px (~${fsPt.toFixed(1)}pt) — below the 8pt readability floor for projected slides.`, { context: { fontSizePx: fsPx, fontSizePt: fsPt } }));
            }
        }
        return out;
    },
        ...
        for (const { node, path } of walk(ctx.tree)) {
            ...
            if (bytes > LARGE_IMAGE_BYTES_THRESHOLD) {
                ...
                if (displayArea < 200_000) {
                    out.push(diag("LARGE_IMAGE_INLINED", "info", ctx, node, path, `<Image> inlines ${(bytes / 1024).toFixed(0)} KB but renders at only ${Math.round(node.w)}x${Math.round(node.h)}px. ` +
                        `Pre-resize the source — every consumer downloads the full payload.`, { context: { bytes, w: node.w, h: node.h } }));
                }
            }
        }
        return out;
...
const excessNodes = {
    ...
    check(ctx) {
        ...
        for (const _entry of walk(ctx.tree))
            count++;
